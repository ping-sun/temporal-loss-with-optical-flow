...
def train(data_loader, net, loss, loss_l1, lamda, beta, optimizer, lr):
    ...
    for i, batch in enumerate(tqdm(data_loader, ncols=80, ascii=True)):
        ...
        prev, cur, heatmaps, flow1, flow2 = batch['prev'], batch['cur'], batch['label'], batch['flow1'], batch['flow2']
        batch_size = len(prev)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    net = network(2)
    #ckpt = torch.load('./ckpt_v0.2.ckpt')['state_dict']
    ...
    ignored_params = list(map(id, net.layer1.parameters()))
    base_params = filter(lambda p: id(p) not in ignored_params, net.parameters())

    optimizer = torch.optim.SGD([
        {'params': base_params, 'lr': lr*0.1, 'name': 'base'},
        {'params': net.layer1.parameters(), 'lr': lr, 'name': 'layer1'},
